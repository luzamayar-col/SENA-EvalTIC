...
export async function POST(req: NextRequest, { params }: Params) {
  ...
  // 1. Verificar que la ficha pertenece al instructor
  const ficha = await prisma.ficha.findFirst({
    where: { id: fichaId, evaluacion: { instructorId: session.user.instructorId } },
    select: {
      numero: true,
      programa: true,
      fechaInicio: true,
  ...
  const { resendApiKey } = ficha.evaluacion.instructor;
  if (!resendApiKey) {
  ...
  // 3. Obtener aprendices con emailPersonal
  const aprendices = await prisma.aprendiz.findMany({
    where: { fichaId },
    select: { nombres: true, apellidos: true, emailPersonal: true },
  });

  const conEmail = aprendices.filter((a) => a.emailPersonal);
  const sinEmail = aprendices.length - conEmail.length;

  if (conEmail.length === 0) {
    return NextResponse.json({ enviados: 0, sinEmail });
  }

  // 4. Preparar datos de envío
  ...
  // Usar fechas de la ficha si existen, si no las de la evaluación
  const fechaInicio = formatFechaEsCO(ficha.fechaInicio ?? ficha.evaluacion.fechaInicio);
  const fechaFin    = formatFechaEsCO(ficha.fechaFin    ?? ficha.evaluacion.fechaFin);

  // 5. Enviar emails uno a uno (Neon HTTP no soporta transacciones, los envíos son independientes)
  let enviados = 0;
  for (const aprendiz of conEmail) {
    const html = buildHtmlNotificacion({
      nombres:         aprendiz.nombres,
      evaluacionNombre: ficha.evaluacion.nombre,
    ...
    try {
      const { error } = await resend.emails.send({
        ...
        subject: `Evaluación disponible: ${ficha.evaluacion.nombre} — Ficha ${ficha.numero}`,
        html,
      });
      if (!error) enviados++;
